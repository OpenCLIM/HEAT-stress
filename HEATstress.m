...
%% Initialise
disp('Running HEAT-metrics v.1.0')
disp('-----')
...
ncwriteatt(fname_long,'/','collection','HEAT-metrics derived heat stress metric')
ncwriteatt(fname_long,'/','creation_date',datestr(now))
%     ncwriteatt(fname_long,'/','domain',fname(11:12))
ncwriteatt(fname_long,'/','title','HEAT-metrcs output')
ncwriteatt(fname_long,'/','version','HEAT-metrics v1.0')
...
%% Finish up
disp(' ')
disp(['HEAT-metrics run "',inputs.ExptName,'" complete',])
endt = now;
